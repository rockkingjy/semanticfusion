...
{
  // CNN Skip params
  const int cnn_skip_frames = 1; //10
  ...
  while (!pangolin::ShouldQuit() && log_reader->hasMore())
  {
    ...
    {
      log_reader->getNext(); // read next frame
      // Map Process----------------------
    ...
    gui->renderMap(map);
    // segmentation output of this frame - left image
    gui->displayRawNetworkPredictions("pred", segmented_prob->mutable_gpu_data());
    // This is to display a predicted semantic segmentation from the fused map - middle image
